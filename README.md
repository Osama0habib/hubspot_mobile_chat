...
| Feature | Android | iOS | Flutter API |
|---------|:-------:|:---:|:-----------:|
| Initialize SDK (`configure`) | ✅ | ✅ | ✅ |
| Open chat (`openChat`, optional `chatFlow`) | ✅ | ✅ | ✅ |
| User identity (`setUserIdentity`) | ✅ | ✅ | ✅ |
| Visitor identification token | ⚠️ | ⚠️ | ✅ |
| Chat properties (`setChatProperties`) | ✅ | ✅ | ✅ |
| Push notifications (`registerPushToken` + `onMessagePush`) | ⚠️ | ⚠️ | ✅ |
| Logout / clear identity (`logout`) | ✅ | ✅ | ✅ |
| Open conversation from push (`openChat(pushData:)`) | ✅ | ✅ | ✅ |
| Programmatic close chat | ❌ | ❌ | ❌ |
| Is-initialized / chat-available check | ❌ | ❌ | ❌ |

Legend: ✅ implemented & wired · ⚠️ implemented, needs extra app/backend setup · ❌ not
offered by the native SDK.

Notes:
- **Visitor identification token** is the `identityToken` argument of `setUserIdentity`. It
  must be generated by **your backend** via HubSpot's Visitor Identification API (it needs
  your private-app secret, which must never live in the app). The plugin only passes it
  through. Without it, chats are anonymous.
- **Push notifications** require your app to own FCM (Android) / APNs (iOS) registration;
  the plugin forwards the token and surfaces new-message events.
- **Programmatic close** and **is-initialized** are not exposed by the upstream native SDKs,
  so the plugin does not offer them. The chat UI is dismissed by the user (Android back /
  iOS sheet dismissal).
...
This plugin only depends on HubSpot's native Mobile Chat SDKs through their official
package managers (Gradle/Maven for Android, Swift Package Manager for iOS) — it does **not**
vendor or redistribute any HubSpot binaries (`.aar`, `.framework`, `.xcframework`). Both
upstream SDKs are MIT-licensed
([Android](https://github.com/HubSpot/mobile-chat-sdk-android),
[iOS](https://github.com/HubSpot/mobile-chat-sdk-ios)). Your use of the HubSpot SDKs is
subject to HubSpot's own terms.
